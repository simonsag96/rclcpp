...
private:
   ...
   * The "validate_and_lock" API allows to restore the heap property and also returns a locked
   * version of the timers heap. This class is not thread safe and requires external mutexes to
   * protect its usage.
   */
  ...
  public:
     ...
     */
    const WeakTimerPtr & front() const { return weak_heap_.front(); }
    /**
     * @brief Returns whether the heap is empty or not.
     */
    bool empty() const { return weak_heap_.empty(); }
    /**
  ...
  public:
     ...
     */
    TimerPtr & front() { return owned_heap_.front(); }
    /**
     * @brief Returns a const reference to the front element.
     * @return const reference to front element.
     */
    const TimerPtr & front() const { return owned_heap_.front(); }
    /**
     * @brief Returns whether the heap is empty or not.
     * @return true if the heap is empty.
     */
    bool empty() const { return owned_heap_.empty(); }
    /**
     * @brief Returns the size of the heap.
     * @return the number of valid timers in the heap.
     */
    size_t size() const { return owned_heap_.size(); }
    /**
     * @brief Get the number of timers that are currently ready.
     * @return size_t number of ready timers.
     */
     ...
     */
    void heapify() { std::make_heap(owned_heap_.begin(), owned_heap_.end(), timer_greater); }
    /**
  ...
  std::function<void(const rclcpp::TimerBase *, const std::shared_ptr<void> &)> on_ready_callback_ =
    nullptr;
  ...
  // Add a time delay backend
  rslcpp::time_delay::DelayBackend & time_delay_backend_ =
    rslcpp::time_delay::DelayBackend::getInstance();
};
}  // namespace experimental
}  // namespace rclcpp
